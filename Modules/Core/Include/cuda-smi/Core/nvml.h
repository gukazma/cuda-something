...
Supported platforms:
- Windows:     Windows Server 2008 R2 64bit, Windows Server 2012 R2 64bit, Windows 7 64bit, Windows
8 64bit
- Linux:       32-bit and 64-bit
- Hypervisors: Windows Server 2008R2/2012 Hyper-V 64bit, Citrix XenServer 6.2 SP1+, VMware
ESX 5.1/5.5
...
On Linux the NVML library will be found on the standard library path. For 64 bit Linux, both the 32
bit and 64 bit NVML libraries will be installed.
...
{
    char busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE];   //!< The tuple domain:bus:device.function PCI
                                                      //!< identifier (&amp; NULL terminator)
 ...
 * Each sample period may be between 1 second and 1/6 second, depending on the product being
 * queried.
 */
typedef struct nvmlUtilization_st
{
    unsigned int gpu;      //!< Percent of time over the past sample period during which one or more
                           //!< kernels was executing on the GPU
    unsigned int memory;   //!< Percent of time over the past sample period during which global
...
typedef struct nvmlMemory_st
{
    ...
    unsigned long long used;    //!< Allocated FB memory (in bytes). Note that the driver/GPU always
                                //!< sets aside a small amount of memory for bookkeeping
} nvmlMemory_t;
 ...
 * bridge is stored at index 0 of bridgeInfoList, parent to immediate bridge is at index 1 and so
 * forth.
 */
typedef struct nvmlBridgeChipHierarchy_st
{
    ...
    nvmlBridgeChipInfo_t
        bridgeChipInfo[NVML_MAX_PHYSICAL_BRIDGE];   //!< Hierarchy of Bridge Chips on the board
...
typedef enum nvmlSamplingType_enum
{
    ...
    NVML_GPU_UTILIZATION_SAMPLES = 1,   //!< To represent percent of time during which one or more
                                        //!< kernels was executing on the GPU
    NVML_MEMORY_UTILIZATION_SAMPLES = 2,   //!< To represent percent of time during which global
                                           //!< (device) memory was being read or written
...
{
    NVML_BRAND_UNKNOWN =
        0,   // This is either Geforce or something else.  NVML does not detect Geforce
    NVML_BRAND_QUADRO = 1,
    NVML_BRAND_TESLA  = 2,
    NVML_BRAND_NVS    = 3,
    NVML_BRAND_GRID   = 4,
...
typedef enum nvmlComputeMode_enum
{
    ...
    NVML_COMPUTEMODE_EXCLUSIVE_THREAD = 1,   //!< Compute-exclusive-thread mode -- only one context
                                             //!< per device, usable from one thread at a time
    NVML_COMPUTEMODE_PROHIBITED = 2,         //!< Compute-prohibited mode -- no contexts per device
    NVML_COMPUTEMODE_EXCLUSIVE_PROCESS =
        3,   //!< Compute-exclusive-process mode -- only one context per device, usable from
 ...
 * ECC counter types.
 *
 * Note: Volatile counts are reset each time the driver loads. On Windows this is once per boot. On
 * Linux this can be more frequent. On Linux the driver unloads when no active clients exist. If
 * persistence mode is enabled or there is always a driver client active (e.g. X11), then Linux also
 * sees per-boot behavior. If not, volatile counts are reset each time a compute app is run.
 */
typedef enum nvmlEccCounterType_enum
{
    NVML_VOLATILE_ECC = 0,   //!< Volatile counts are reset each time the driver loads.
    NVML_AGGREGATE_ECC =
        1,   //!< Aggregate counts persist across reboots (i.e. for the lifetime of the device)
...
typedef enum nvmlMemoryLocation_enum
{
    ...
    // Keep this last
    NVML_MEMORY_LOCATION_COUNT   //!< This counts the number of memory locations the driver knows
                                 //!< about
...
typedef enum nvmlRestrictedAPI_enum
{
    NVML_RESTRICTED_API_SET_APPLICATION_CLOCKS =
        0,   //!< APIs that change application clocks, see nvmlDeviceSetApplicationsClocks
 ...
 *
 * See \ref nvmlDeviceRegisterEvents and \ref nvmlDeviceGetSupportedEventTypes to check which
 * devices support each event.
 *
...
/**
 * \note A corrected texture memory error is not an ECC error, so it does not generate a single bit
 * event
 */
...
/**
 * \note An uncorrected texture memory error is not an ECC error, so it does not generate a double
 * bit event
 */
 ...
 * to no work being executed on the GPU, power capping or thermal capping. In a typical situation,
...
typedef struct nvmlEventData_st
{
    ...
    unsigned long long eventData;   //!< Stores last XID error for the device in the event of
                                    //!< nvmlEventTypeXidCriticalError,
                                    //  eventData is 0 for any other event. eventData is set as 999
...
#define nvmlClocksThrottleReasonAll                                                           \
    (nvmlClocksThrottleReasonNone | nvmlClocksThrottleReasonGpuIdle |                         \
     nvmlClocksThrottleReasonApplicationsClocksSetting | nvmlClocksThrottleReasonSwPowerCap | \
     nvmlClocksThrottleReasonHwSlowdown | nvmlClocksThrottleReasonUnknown)
/** @} */
...
{
    unsigned int
        gpuUtilization;   //!< Percent of time over the process's lifetime during which one or more
                          //!< kernels was executing on the GPU.
                          //! Utilization stats just like returned by \ref
                          //! nvmlDeviceGetUtilizationRates but for the life time of a process (not
                          //! just the last sample period). Set to NVML_VALUE_NOT_AVAILABLE if
                          //! nvmlDeviceGetUtilizationRates is not supported

    unsigned int memoryUtilization;   //!< Percent of time over the process's lifetime during which
                                      //!< global (device) memory was being read or written.
                                      //! Set to NVML_VALUE_NOT_AVAILABLE if
                                      //! nvmlDeviceGetUtilizationRates is not supported

    unsigned long long
        maxMemoryUsage;   //!< Maximum total memory in bytes that was ever allocated by the process.
                          //! Set to NVML_VALUE_NOT_AVAILABLE if nvmlProcessInfo_t->usedGpuMemory is
                          //! not supported
 ...
 *
 * \note In NVML 5.319 new nvmlInit_v2 has replaced nvmlInit"_v1" (default in NVML 4.304 and older)
 * that did initialize all GPU devices in the system.
 *
 ...
 *
 * \note To contrast nvmlInit_v2 with nvmlInit"_v1", NVML 4.304 nvmlInit"_v1" will fail when any
 * detected GPU is in a bad or unstable state.
 *
 * For all products.
 *
 ...
 *         - \ref NVML_ERROR_NO_PERMISSION       if NVML does not have permission to talk to the
 * driver
...
/**
 * Buffer size guaranteed to be large enough for \ref nvmlDeviceGetInforomVersion and \ref
 ...
 * @param length                               The maximum allowed length of the string returned in
 * \a version
 *
 * @return
 ...
 *         - \ref NVML_ERROR_NO_PERMISSION     if the user doesn't have permission to perform this
 * operation
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
 ...
 * This chapter describes that queries that NVML can perform against each unit. For S-class systems
 * only. In each case the device is identified with an nvmlUnit_t handle. This handle is obtained by
 ...
 *   For example, if \a unitCount is 2 the valid indices are 0 and 1, corresponding to UNIT 0 and
 * UNIT 1.
 *
 ...
 *
 * @param unit                                 The identifier of the target unit
 * @param fanSpeeds                            Reference in which to return the fan speed
 * information
 *
 * @return
 ...
 *
 * @param unit                                 The identifier of the target unit
 * @param deviceCount                          Reference in which to provide the \a devices array
 * size, and to return the number of attached GPU devices
 * @param devices                              Reference in which to return the references to the
 * attached GPU devices
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a deviceCount and \a devices have been populated
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INSUFFICIENT_SIZE if \a deviceCount indicates that the \a devices array
 * is too small
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a unit is invalid, either of \a deviceCount or \a
 * devices is NULL
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlUnitGetDevices(nvmlUnit_t unit, unsigned int* deviceCount,
 ...
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a hwbcCount and \a hwbcEntries have been
 * populated
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if either \a hwbcCount or \a hwbcEntries is NULL
 *         - \ref NVML_ERROR_INSUFFICIENT_SIZE if \a hwbcCount indicates that the \a hwbcEntries
 * array is too small
 */
 ...
 * For all products.
 *
 * Note: New nvmlDeviceGetCount_v2 (default in NVML 5.319) returns count of all devices in the
 * system even if nvmlDeviceGetHandleByIndex_v2 returns NVML_ERROR_NO_PERMISSION for such device.
 *       Update your code to handle this error, or use NVML 4.304 or older nvml header file.
 *       For backward binary compatibility reasons _v1 version of the API is still present in the
 * shared library. Old _v1 version of nvmlDeviceGetCount doesn't count devices that NVML has no
 * permission to talk to.
 *
 * @param deviceCount                          Reference in which to return the number of accessible
 * devices
 *
 * @return
 ...
 *
 * The order in which NVML enumerates devices has no guarantees of consistency between reboots. For
 * that reason it is recommended that devices be looked up by their PCI ids or UUID. See \ref
 * nvmlDeviceGetHandleByUUID() and \ref nvmlDeviceGetHandleByPciBusId().
 *
 ...
 * permission to talk to.
 *
 *       This means that nvmlDeviceGetHandleByIndex_v2 and _v1 can return different devices for the
 * same index. If you don't touch macros that map old (_v1) versions to _v2 versions at the top of
 * the file you don't need to worry about that.
 *
 * @param index                                The index of the target GPU, >= 0 and < \a
 * accessibleDevices
 ...
 *         - \ref NVML_ERROR_INSUFFICIENT_POWER if any attached devices have improperly attached
 * external power cables
 *         - \ref NVML_ERROR_NO_PERMISSION      if the user doesn't have permission to talk to this
 * device
 *         - \ref NVML_ERROR_IRQ_ISSUE          if NVIDIA kernel detected an interrupt issue with
 * the attached GPUs
 ...
 *
 * @deprecated Since more than one GPU can exist on a single board this function is deprecated in
 * favor of \ref nvmlDeviceGetHandleByUUID. For dual GPU boards this function will return
 ...
 *         - \ref NVML_SUCCESS                  if \a device has been set
 *         - \ref NVML_ERROR_UNINITIALIZED      if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT   if \a serial is invalid, \a device is NULL or more
 * than one device has the same serial (dual GPU boards)
 *         - \ref NVML_ERROR_NOT_FOUND          if \a serial does not match a valid device on the
 * system
 *         - \ref NVML_ERROR_INSUFFICIENT_POWER if any attached devices have improperly attached
 * external power cables
 *         - \ref NVML_ERROR_IRQ_ISSUE          if NVIDIA kernel detected an interrupt issue with
 * the attached GPUs
 *         - \ref NVML_ERROR_GPU_IS_LOST        if any GPU has fallen off the bus or is otherwise
 * inaccessible
 *         - \ref NVML_ERROR_UNKNOWN            on any unexpected error
 *
 ...
 *         - \ref NVML_ERROR_NOT_FOUND          if \a uuid does not match a valid device on the
 * system
 *         - \ref NVML_ERROR_INSUFFICIENT_POWER if any attached devices have improperly attached
 * external power cables
 *         - \ref NVML_ERROR_IRQ_ISSUE          if NVIDIA kernel detected an interrupt issue with
 ...
 *  - The target GPU is an SLI slave
 *
 * \note NVML 4.304 and older version of nvmlDeviceGetHandleByPciBusId"_v1" returns
 ...
 *         - \ref NVML_ERROR_NOT_FOUND          if \a pciBusId does not match a valid device on the
 * system
 *         - \ref NVML_ERROR_INSUFFICIENT_POWER if the attached device has improperly attached
 * external power cables
 *         - \ref NVML_ERROR_NO_PERMISSION      if the user doesn't have permission to talk to this
 * device
 *         - \ref NVML_ERROR_IRQ_ISSUE          if NVIDIA kernel detected an interrupt issue with
 ...
 * For all products.
 *
 * The name is an alphanumeric string that denotes a particular product, e.g. Tesla &tm; C2070. It
 * will not exceed 64 characters in length (including the NULL terminator).  See \ref
 ...
 *
 * The order in which NVML enumerates devices has no guarantees of consistency between reboots. For
 * that reason it is recommended that devices be looked up by their PCI ids or GPU UUID. See \ref
 * nvmlDeviceGetHandleByPciBusId() and \ref nvmlDeviceGetHandleByUUID().
 *
 * Note: The NVML index may not correlate with other APIs, such as the CUDA device index.
 *
 * @param device                               The identifier of the target device
 * @param index                                Reference in which to return the NVML index of the
 * device
 *
 * @return
 ...
 *
 * The serial number is an alphanumeric string that will not exceed 30 characters (including the
 * NULL terminator). This number matches the serial number tag that is physically attached to the
 * board.  See \ref nvmlConstants::NVML_DEVICE_SERIAL_BUFFER_SIZE.
 *
 * @param device                               The identifier of the target device
 * @param serial                               Reference in which to return the board/module serial
 * number
 * @param length                               The maximum allowed length of the string returned in
 * \a serial
 *
 ...
 * for the device For example, if processors 0, 1, 32, and 33 are ideal for the device and
 ...
 * @param cpuSet                               Array reference in which to return a bitmask of CPUs,
 * 32 CPUs per unsigned int
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a cpuAffinity has been filled
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, cpuSetSize == 0, or cpuSet
 * is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetCpuAffinity(nvmlDevice_t device, unsigned int cpuSetSize,
                                              unsigned int* cpuSet);
...
/**
 * Retrieves the globally unique immutable UUID associated with this device, as a 5 part hexadecimal
 * string, that augments the immutable, board serial identifier.
 *
 * For all CUDA capable GPUs.
 *
 * The UUID is a globally unique identifier. It is the only available identifier for
 * pre-Fermi-architecture products. It does NOT correspond to any identifier printed on the board.
 * It will not exceed 80 characters in length (including the NULL terminator).  See \ref
...
/**
 * Retrieves minor number for the device. The minor number for the device is such that the Nvidia
 * device node file for each GPU will have the form /dev/nvidia[minor number].
 *
 * For all the GPUs. Supported only for Linux
 *
 * @param device                                The identifier of the target device
 * @param minorNumber                           Reference in which to return the minor number for
 * the device
 * @return
 ...
 * For Tesla &tm; and Quadro &reg; products from the Fermi and Kepler families.
 *
 * Fermi and higher parts have non-volatile on-board memory for persisting device info, such as
 * aggregate ECC counts. The version of the data structures in this memory may change from time to
 * time. It will not exceed 16 characters in length (including the NULL terminator). See \ref
 ...
 */
nvmlReturn_t DECLDIR nvmlDeviceGetInforomVersion(nvmlDevice_t device, nvmlInforomObject_t object,
 ...
 *
 * Image version just like VBIOS version uniquely describes the exact version of the infoROM flashed
 * on the board in contrast to infoROM object version which is only an indicator of supported
 * features. Version string will not exceed 16 characters in length (including the NULL terminator).
 * See \ref nvmlConstants::NVML_DEVICE_INFOROM_VERSION_BUFFER_SIZE.
 *
 * @param device                               The identifier of the target device
 * @param version                              Reference in which to return the infoROM image
 * version
 * @param length                               The maximum allowed length of the string returned in
 * \a version
 *
 ...
 *
 * @param device                               The identifier of the target device
 * @param checksum                             Reference in which to return the infoROM
 * configuration checksum
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a checksum has been set
 *         - \ref NVML_ERROR_CORRUPTED_INFOROM if the device's checksum couldn't be retrieved due to
 * infoROM corruption
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetInforomConfigurationChecksum(nvmlDevice_t  device,
 ...
 *
 * @param device                               The identifier of the target device
 * @param mode                                 Reference in which to return the current driver
 * persistence mode
 *
 * @return
 ...
 *
 * I.E. for a generation 2 PCIe device attached to a generation 1 PCIe bus the max link generation
 * this function will report is generation 1.
 *
 * For Tesla &tm; and Quadro &reg; products from the Fermi and Kepler families.
 *
 * @param device                               The identifier of the target device
 * @param maxLinkGen                           Reference in which to return the max PCIe link
 * generation
 *
 * @return
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetMaxPcieLinkGeneration(nvmlDevice_t  device,
                                                        unsigned int* maxLinkGen);

/**
 * Retrieves the maximum PCIe link width possible with this device and system
 *
 * I.E. for a device with a 16x PCIe bus width attached to a 8x PCIe system bus this function will
 * report a max link width of 8.
 *
 * For Tesla &tm; and Quadro &reg; products from the Fermi and Kepler families.
 *
 * @param device                               The identifier of the target device
 * @param maxLinkWidth                         Reference in which to return the max PCIe link
 * generation
 *
 * @return
 ...
 *
 * @param device                               The identifier of the target device
 * @param currLinkGen                          Reference in which to return the current PCIe link
 * generation
 *
 * @return
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetCurrPcieLinkGeneration(nvmlDevice_t  device,
 ...
 *
 * @param device                               The identifier of the target device
 * @param currLinkWidth                        Reference in which to return the current PCIe link
 * generation
 *
 * @return
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetCurrPcieLinkWidth(nvmlDevice_t  device,
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetClockInfo(nvmlDevice_t device, nvmlClockType_t type,
                                            unsigned int* clock);
 ...
 * See \ref nvmlClockType_t for details on available clock information.
 *
 * \note On GPUs from Fermi family current P0 clocks (reported by \ref nvmlDeviceGetClockInfo) can
...
/**
 * Retrieves the current setting of a clock that applications will use unless an overspec situation
 * occurs. Can be changed using \ref nvmlDeviceSetApplicationsClocks.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a clockMHz is NULL or \a
 * clockType is invalid
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 * \see nvmlDeviceGetApplicationsClock
 */
nvmlReturn_t DECLDIR nvmlDeviceGetDefaultApplicationsClock(nvmlDevice_t    device,
 ...
 * Default value is constant, but the current value an be changed using \ref
...
/**
 * Retrieves the list of possible memory clocks that can be used as an argument for \ref
 * nvmlDeviceSetApplicationsClocks.
 *
 * For Tesla &tm; products from the Kepler family.
 *
 * @param device                               The identifier of the target device
 * @param count                                Reference in which to provide the \a clocksMHz array
 * size, and to return the number of elements
 ...
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_INSUFFICIENT_SIZE if \a count is too small (\a count is set to the
...
/**
 * Retrieves the list of possible graphics clocks that can be used as an argument for \ref
 * nvmlDeviceSetApplicationsClocks.
 *
 * For Tesla &tm; products and Quadro &reg; products from the Kepler family.
 *
 * @param device                               The identifier of the target device
 * @param memoryClockMHz                       Memory clock for which to return possible graphics
 * clocks
 * @param count                                Reference in which to provide the \a clocksMHz array
 * size, and to return the number of elements
 ...
 *         - \ref NVML_SUCCESS                 if \a count and \a clocksMHz have been populated
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_NOT_FOUND         if the specified \a memoryClockMHz is not a supported
 * frequency
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a clock is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 ...
 */
nvmlReturn_t DECLDIR nvmlDeviceGetSupportedGraphicsClocks(nvmlDevice_t  device,
 ...
 *
 * Auto boosted clocks are enabled by default on some hardware, allowing the GPU to run at higher
 * clock rates to maximize performance as thermal limits allow.
 *
 * @param device                               The identifier of the target device
 * @param isEnabled                            Where to store the current state of auto boosted
 * clocks of the target device
 * @param defaultIsEnabled                     Where to store the default auto boosted clocks
 * behavior of the target device that the device will revert to when no applications are using the
 * GPU
 *
 * @return
 *         - \ref NVML_SUCCESS                 If \a isEnabled has been been set with the auto
 * boosted clocks state of \a device
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 ...
 *
 * Auto boosted clocks are enabled by default on some hardware, allowing the GPU to run at higher
 * clock rates to maximize performance as thermal limits allow. Auto boosted clocks should be
 * disabled if fixed clock rates are desired. For non-root users, the user must be first allowed to
 * use this API with nvmlDeviceSetAPIRestriction, with
 * apiType=NVML_RESTRICTED_API_SET_AUTO_BOOSTED_CLOCKS.
 *
 * @param device                               The identifier of the target device
 * @param enabled                              What state to try to set auto boosted clocks of the
 * target device to
 *
 * @return
 *         - \ref NVML_SUCCESS                 If the auto boosted clocks were successfully set to
 * the state specified by \a enabled
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support auto boosted clocks
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 ...
 * auto boosted clocks will return to when no compute running processes (e.g. CUDA application which
 * have an active context) are running
 *
 * For Tesla &tm; products of Kepler or newer
 ...
 * Auto boosted clocks are enabled by default on some hardware, allowing the GPU to run at higher
 * clock rates to maximize performance as thermal limits allow. Auto boosted clocks should be
 * disabled if fixed clock rates are desired.
 *
 * @param device                               The identifier of the target device
 * @param enabled                              What state to try to set default auto boosted clocks
 * of the target device to
 * @param flags                                Flags that change the default behavior. nvmlFlagForce
 * will make the current auto boost mode the new default mode immediately rather than waiting for no
 * compute processes to be running.
 *
 * @return
 *         - \ref NVML_SUCCESS                 If the auto boosted clock's default state was
 * successfully set to the state specified by \a enabled
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_NO_PERMISSION     If the calling user does not have permission to
 ...
 * to spin, the output will not match the actual fan speed.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a sensorType is invalid or
 * \a temp is NULL
 ...
 *         - \ref NVML_SUCCESS                 if \a temp has been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a thresholdType is invalid
 * or \a temp is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not have a temperature sensor or
 * is unsupported
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 *
 * @param device                               The identifier of the target device
 * @param pState                               Reference in which to return the performance state
 * reading
 *
 * @return
 ...
 *
 * \note More than one bit can be enabled at the same time. Multiple reasons can be affecting clocks
 * at once.
 *
 * @param device                                The identifier of the target device
 * @param clocksThrottleReasons                 Reference in which to return bitmask of active
 * clocks throttle reasons
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a clocksThrottleReasons has been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a clocksThrottleReasons
 * is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 ...
 * supportedClocksThrottleReasons is NULL
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 *
...
/**
 * Deprecated: Use \ref nvmlDeviceGetPerformanceState. This function exposes an incorrect
 * generalization.
 *
 ...
 *
 * @param device                               The identifier of the target device
 * @param mode                                 Reference in which to return the current power
 * management mode
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a mode has been set
 ...
 *
 * @param device                               The identifier of the target device
 * @param limit                                Reference in which to return the power management
 * limit in milliwatts
 *
 * @return
 ...
 *
 * @param device                               The identifier of the target device
 * @param minLimit                             Reference in which to return the minimum power
 * management limit in milliwatts
 * @param maxLimit                             Reference in which to return the maximum power
 * management limit in milliwatts
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a minLimit and \a maxLimit have been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a minLimit or \a maxLimit
 * is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 ...
 *
 * @param device                               The identifier of the target device
 * @param defaultLimit                         Reference in which to return the default power
 * management limit in milliwatts
 *
 * @return
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetPowerManagementDefaultLimit(nvmlDevice_t  device,
 ...
 *
 * @param device                               The identifier of the target device
 * @param power                                Reference in which to return the power usage
 * information
 *
 * @return
 ...
 *
 * Note: This can be different from the \ref nvmlDeviceGetPowerManagementLimit if other limits are
 * set elsewhere This includes the out of band power limit interface
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a current or \a pending
 * is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 ...
 */
nvmlReturn_t DECLDIR nvmlDeviceGetGpuOperationMode(nvmlDevice_t            device,
 ...
 *
 * Under Linux and Windows TCC, the reported amount of used memory is equal to the sum of memory
 * allocated by all active channels on the device.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or either \a current or \a
 * pending is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 ...
 *  The boardId returned is a unique ID for the current configuration.  Uniqueness and ordering
 * across reboots and system configurations is not guaranteed (i.e. if a Tesla K40c returns 0x100
 * and the two GPUs on a Tesla K10 in the same system returns 0x200 it is not guaranteed they will
 ...
 *                                                 to indicate whether the device is on a multi GPU
 * board
 *
 * @return
 ...
 *
 * The total error count is the sum of errors across each of the separate memory systems, i.e. the
 * total set of errors across the entire device.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device, \a errorType or \a counterType is
 * invalid, or \a eccCounts is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 *
 * Reports zero for unsupported ECC error counters when a subset of ECC error counters are
 * supported.
 *
 ...
 * Requires \a NVML_INFOROM_ECC version 2.0 or higher to report aggregate location-based memory
 * error counts. Requires \a NVML_INFOROM_ECC version 1.0 or higher to report all other memory error
 * counts.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device, \a bitTyp,e \a counterType or \a
 * locationType is invalid, or \a count is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support ECC error reporting in
 ...
 *
 * \note During driver initialization when ECC is enabled one can see high GPU and Memory
 * Utilization readings. This is caused by ECC Memory Scrubbing mechanism that is performed during
 * driver initialization.
 *
 * @param device                               The identifier of the target device
 * @param utilization                          Reference in which to return the utilization
 * information
 *
 * @return
 ...
 *
 * @param device                               The identifier of the target device
 * @param utilization                          Reference to an unsigned int for encoder utilization
 * info
 * @param samplingPeriodUs                     Reference to an unsigned int for the sampling period
 * in US
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a utilization has been populated
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a utilization is NULL, or
 * \a samplingPeriodUs is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 *
 * @param device                               The identifier of the target device
 * @param utilization                          Reference to an unsigned int for decoder utilization
 * info
 * @param samplingPeriodUs                     Reference to an unsigned int for the sampling period
 * in US
 *
 ...
 * For windows only.
 *
 * On Windows platforms the device driver can run in either WDDM or WDM (TCC) mode. If a display is
 * attached to the device it must run in WDDM mode. TCC mode is preferred if a display is not
 * attached.
 *
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or both \a current and \a
 * pending are NULL
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetVbiosVersion(nvmlDevice_t device, char* version,
                                               unsigned int length);
 ...
 *
 */
nvmlReturn_t DECLDIR nvmlDeviceGetBridgeChipInfo(nvmlDevice_t               device,
 ...
 * For Tesla &tm; and Quadro &reg; products from the Fermi and Kepler families.
 *
 * This function returns information only about compute running processes (e.g. CUDA application
 * which have active context). Any graphics applications (e.g. using OpenGL, DirectX) won't be
 * listed by this function.
 *
 * To query the current number of running compute processes, call this function with *infoCount = 0.
 * The return code will be NVML_ERROR_INSUFFICIENT_SIZE, or NVML_SUCCESS if none are running. For
 * this call \a infos is allowed to be NULL.
 *
 * Keep in mind that information returned by this call is dynamic and the number of elements might
 * change in time. Allocate more space for \a infos table in case new compute processes are spawned.
 *
 * @param device                               The identifier of the target device
 * @param infoCount                            Reference in which to provide the \a infos array
 * size, and to return the number of returned elements
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INSUFFICIENT_SIZE if \a infoCount indicates that the \a infos array is
 * too small \a infoCount will contain minimal amount of space necessary for the call to complete
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, either of \a infoCount or \a
 * infos is NULL
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 *                                              Non-zero indicates that the GPUs are on the same
 * board.
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a onSameBoard has been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a dev1 or \a dev2 are invalid or \a onSameBoard
 * is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if this check is not supported by the device
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the either GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceOnSameBoard(nvmlDevice_t device1, nvmlDevice_t device2,
                                           int* onSameBoard);

/**
 * Retrieves the root/admin permissions on the target API. See \a nvmlRestrictedAPI_t for the list
 * of supported APIs. If an API is restricted only root users can call that API. See \a
 ...
 *                                             NVML_FEATURE_ENABLED indicates that the API is
 * root-only NVML_FEATURE_DISABLED indicates that the API is accessible to all users
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a isRestricted has been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a apiType incorrect or \a
 * isRestricted is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if this query is not supported by the device or the
 * device does not support the feature that is being queried (E.G. Enabling/disabling auto boosted
 * clocks is not supported by the device)
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 *
 ...
 * Based on type, this method can be used to fetch the power, utilization or clock samples
 maintained in the buffer by
 * the driver.
 *
 * Power, Utilization and Clock samples are returned as type "unsigned int" for the union
 nvmlValue_t.
 *
 * To get the size of samples that user needs to allocate, the method is invoked with samples set to
 NULL.
 * The returned samplesCount will provide the number of samples that can be queried. The user needs
 to
 ...
 * lastSeenTimeStamp represents CPU timestamp in microseconds. Set it to 0 to fetch all the samples
 maintained by the
 * underlying buffer. Set lastSeenTimeStamp to one of the timeStamps retrieved from the date of the
 previous query
 * to get more recent samples.
 *
 * This method fetches the number of entries which can be accommodated in the provided samples
 array, and the
 * reference samplesCount is updated to indicate how many samples were actually retrieved. The
 advantage of using this
 * method for samples in contrast to polling via existing methods is to get get higher frequency
 data at lower polling cost.
 *
 * @param device                        The identifier for the target device
 * @param type                          Type of sampling event
 * @param lastSeenTimeStamp             Return only samples with timestamp greater than
 lastSeenTimeStamp.
 * @param sampleValType                 Output parameter to represent the type of sample value as
 described in nvmlSampleVal_t
 * @param sampleCount                   Reference to provide the number of elements which can be
 queried in samples array
 ...
 *                                             reference to \a sampleCount is 0 for non null \a
 samples
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if this query is not supported by the device
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 otherwise inaccessible
 ...
 *
 * BAR1 is used to map the FB (device memory) so that it can be directly accessed by the CPU or by
 * 3rd party devices (peer-to-peer on the PCIE bus).
 *
 * For Tesla and Quadro &tm products from the Kepler+ family.
 *
 * @param device                               The identifier of the target device
...
/**
 * Gets the duration of time during which the device was throttled (lower than requested clocks) due
 * to power or thermal constraints.
 *
 * The method is important to users who are tying to understand if their GPUs throttle at any point
 * during their applications. The difference in violation times at two different reference times
 * gives the indication of GPU throttling event.
 *
 ...
 *
 * @param device                               The identifier of the target device
 * @param perfPolicyType                       Represents Performance policy which can trigger GPU
 * throttling
 * @param violTime                             Reference to which violation time related information
 * is returned
 *
 *
 * @return
 ...
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a perfPolicyType is
 * invalid, or \a violTime is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if this query is not supported by the device
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *
 */
nvmlReturn_t DECLDIR nvmlDeviceGetViolationStatus(nvmlDevice_t         device,
 ...
 *
 * @param device                               The identifier of the target device
 * @param mode                                 Reference in which to return the current accounting
 * mode
 *
 * @return
 ...
 * Accounting stats are kept in a circular buffer, newly created processes overwrite information
 * about old processes.
 *
 ...
 * @note Only compute and graphics applications stats can be queried. Monitoring applications stats
 * can't be queried since they don't contribute to GPU utilization.
 * @note In case of pid collision stats of only the latest process (that terminated last) will be
 * reported
 *
 * @warning On Kepler devices per process statistics are accurate only if there's one process
 * running on a GPU.
 *
 * @param device                               The identifier of the target device
 * @param pid                                  Process Id of the target process to query stats for
 * @param stats                                Reference in which to return the process's accounting
 * stats
 *
 * @return
 ...
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device doesn't support this feature or
 ...
 * pids=NULL. The return code will be NVML_ERROR_INSUFFICIENT_SIZE, or NVML_SUCCESS if list is
 * empty.
 *
 * For more details see \ref nvmlDeviceGetAccountingStats.
 *
 * @note In case of PID collision some processes might not be accessible before the circular buffer
 * is full.
 *
 * @param device                               The identifier of the target device
 * @param count                                Reference in which to provide the \a pids array size,
 * and to return the number of elements ready to be queried
 ...
 * For Tesla &tm; and Quadro &reg; products from the Kepler family.
 *
 * This is the maximum number of processes that accounting information will be stored for before
 * information about oldest processes will get overwritten by information about new processes.
 *
 * @param device                               The identifier of the target device
 * @param bufferSize                           Reference in which to provide the size (in number of
 ...
 */
nvmlReturn_t DECLDIR nvmlDeviceGetAccountingBufferSize(nvmlDevice_t  device,
 ...
 * The address information provided from this API is the hardware address of the page that was
 * retired.  Note that this does not match the virtual address used in CUDA, but will match the
 ...
 * @param pageCount                         Reference in which to provide the \a addresses buffer
 * size, and to return the number of retired pages that match \a cause Set to 0 to query the size
 * without allocating an \a addresses buffer
 * @param addresses                         Buffer to write the page addresses into
 *
 * @return
 *         - \ref NVML_SUCCESS                 if \a pageCount was populated and \a addresses was
 * filled
 *         - \ref NVML_ERROR_INSUFFICIENT_SIZE if \a pageCount indicates the buffer is not large
 * enough to store all the matching page addresses.  \a pageCount is set to the needed size.
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid, \a pageCount is NULL, \a
 * cause is invalid, or \a addresses is NULL
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device doesn't support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceGetRetiredPagesPendingStatus(nvmlDevice_t       device,
 ...
 * and back LEDs will be toggled in unison regardless of which unit is specified with this
 * command.</b>
 *
 ...
 * Requires root/admin permissions.
 *
 * The persistence mode determines whether the GPU driver software is torn down after the last
 * client exits.
 *
 * This operation takes effect immediately. It is not persistent across reboots. After each reboot
 * the persistence mode is reset to "Disabled".
 *
 ...
 *
 * @param device                               The identifier of the target device
 * @param counterType                          Flag that indicates which type of errors should be
 * cleared.
 *
 * @return
 ...
 *
 * On Windows platforms the device driver can run in either WDDM or WDM (TCC) mode. If a display is
 * attached to the device it must run in WDDM mode.
 *
 * It is possible to force the change to WDM (TCC) while the display is still attached with a force
 * flag (nvmlFlagForce). This should only be done if the host is subsequently powered down and the
 * display is detached from the device before the next reboot.
 *
 * This operation takes effect after the next reboot.
 *
 ...
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the platform is not windows or the device does not
 ...
 *         - \ref NVML_SUCCESS                 if new settings were successfully set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a memClockMHz and \a
 ...
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 */
nvmlReturn_t DECLDIR nvmlDeviceSetApplicationsClocks(nvmlDevice_t device, unsigned int memClockMHz,
 ...
 *         - \ref NVML_SUCCESS                 if \a limit has been set
 *         - \ref NVML_ERROR_UNINITIALIZED     if the library has not been successfully initialized
 *         - \ref NVML_ERROR_INVALID_ARGUMENT  if \a device is invalid or \a defaultLimit is out of
 * range
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support this feature
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
...
/**
 * Changes the root/admin restructions on certain APIs. See \a nvmlRestrictedAPI_t for the list of
 * supported APIs. This method can be used by a root/admin user to give non-root/admin access to
 * certain otherwise-restricted APIs. The new setting lasts for the lifetime of the NVIDIA driver;
 * it is not persistent. See \a nvmlDeviceGetAPIRestriction to query the current restriction
 * settings.
 *
 * For Tesla and Quadro &tm products from the Kepler+ family.
 * Requires root/admin permissions.
 ...
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the device does not support changing API
 * restrictions or the device does not support the feature that api restrictions are being set for
 ...
 * @see nvmlRestrictedAPI_t
 */
nvmlReturn_t DECLDIR nvmlDeviceSetAPIRestriction(nvmlDevice_t device, nvmlRestrictedAPI_t apiType,
                                                 nvmlEnableState_t isRestricted);
...
/**
 * Starts recording of events on a specified devices and add the events to specified \ref
 * nvmlEventSet_t
 *
 * For Tesla &tm; and Quadro &reg; products from the Fermi and Kepler families.
 ...
 * If function reports NVML_ERROR_NOT_SUPPORTED, event set can still be used. None of the requested
 * eventTypes are registered in that case.
 *
 * @param device                               The identifier of the target device
 ...
 *         - \ref NVML_ERROR_NOT_SUPPORTED     if the platform does not support this feature or some
 * of requested event types
 *         - \ref NVML_ERROR_GPU_IS_LOST       if the target GPU has fallen off the bus or is
 * otherwise inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 *
 ...
 * @see nvmlEventSetFree
 */
nvmlReturn_t DECLDIR nvmlDeviceRegisterEvents(nvmlDevice_t device, unsigned long long eventTypes,
                                              nvmlEventSet_t set);
 ...
 * For all products.
 *
 * Events are not supported on Windows. So this function returns an empty mask in \a eventTypes on
 * Windows.
 *
 * @param device                               The identifier of the target device
 * @param eventTypes                           Reference in which to return bitmask of supported
 * events
 *
 * @return
 ...
 */
nvmlReturn_t DECLDIR nvmlDeviceGetSupportedEventTypes(nvmlDevice_t        device,
 ...
 *
 * In case of xid error, the function returns the most recent xid error type seen by the system. If
 * there are multiple xid errors generated before nvmlEventSetWait is invoked then the last seen xid
 ...
 * @param timeoutms                            Maximum amount of wait time in milliseconds for
 * registered event
 *
 * @return
 ...
 *         - \ref NVML_ERROR_TIMEOUT           if no event arrived in specified timeout or interrupt
 * arrived
 *         - \ref NVML_ERROR_GPU_IS_LOST       if a GPU has fallen off the bus or is otherwise
 * inaccessible
 *         - \ref NVML_ERROR_UNKNOWN           on any unexpected error
 *
 * @see nvmlEventType
 * @see nvmlDeviceRegisterEvents
 */
nvmlReturn_t DECLDIR nvmlEventSetWait(nvmlEventSet_t set, nvmlEventData_t* data,
